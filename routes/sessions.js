...
exports.findAll = function (req, res, next) {
    ...
    if (name) {
        res.send(sessions.filter(function(session) {
            return (session.firstName + ' ' + session.lastName).toLowerCase().indexOf(name.toLowerCase()) > -1;
        }));
    } else {
        res.send(sessions);
    }
...
exports.findReports = function (req, res, next) {
    ...
    response = {
        id: id,
        firstName: sessions[id].firstName,
        lastName: sessions[id].lastName,
    ...
    for (var i=0; i<sessions.length; i++) {
        session = sessions[i];
        if (session.managerId === id) {
            reports.push({id: session.id, firstName: session.firstName, lastName: session.lastName, title: session.title, pic: session.pic});
        }
    }
